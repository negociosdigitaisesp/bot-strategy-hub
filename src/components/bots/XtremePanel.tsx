...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Target, CheckCircle2, XCircle, AlertCircle, ArrowRight, Flame } from 'lucide-react';
import { cn } from '../../lib/utils';
import { toast } from 'sonner';
import { useBotXtreme } from '../../hooks/useBotXtreme';
import { useDeriv } from '../../contexts/DerivContext';

interface XtremePanelProps {
    isActive: boolean;
    onToggle: () => void;
    onBack: () => void;
}

export const XtremePanel: React.FC<XtremePanelProps> = ({ isActive, onToggle, onBack }) => {
    const { isConnected } = useDeriv();
    const { isRunning, stats, logs, startBot, stopBot } = useBotXtreme();

    // Config States (no last digit field - hidden strategy)
    ...
    const handleToggleBot = () => {
        if (isRunning) {
            stopBot();
            toast.info('Xtreme Bot detenido');
        } else {
            if (!isConnected) {
            ...
            if (success) {
                toast.success('Xtreme Bot iniciado');
            }
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#0a0a0f] via-[#0f0f14] to-[#0a0a0f] p-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-fuchsia-500 to-purple-600 flex items-center justify-center border border-fuchsia-400/20">
                                <Flame className="text-white" size={28} />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Xtreme Bot</h1>
                            <p className="text-sm text-white/40">Estrategia DIGITDIFF de alto rendimiento</p>
                        </div>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            {isRunning && (
                                <span className="absolute inset-0 rounded-full bg-fuchsia-500 animate-ping" />
                            )}
                            <span className={cn(
                                "absolute inset-0 rounded-full",
                                isRunning ? "bg-fuchsia-500" : "bg-gray-500"
                            )} />
                        </div>
                        <span className={cn(
                            "text-sm font-bold uppercase tracking-wider",
                            isRunning ? "text-fuchsia-400" : "text-white/50"
                        )}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-fuchsia-500/50 focus:ring-2 focus:ring-fuchsia-500/20 transition-all outline-none disabled:opacity-50"
                                        step="0.01"
                                    />
                                </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isRunning
                                    ...
                                    : "bg-gradient-to-r from-fuchsia-600 to-purple-600 hover:from-fuchsia-500 hover:to-purple-500 text-white border-none shadow-lg shadow-fuchsia-500/25"
                            )}
                        >
                            {isRunning ? (
                                <>
                    ...
                    <div className="col-span-12 lg:col-span-9 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            <div className="bg-gradient-to-br from-white/[0.05] to-white/[0.02] border border-white/10 rounded-2xl p-6 relative overflow-hidden group hover:border-white/20 transition-all">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-fuchsia-500/10 to-transparent rounded-full blur-3xl" />
                                <div className="relative">
                            ...
                            {/* Total Operations */}
                            <div className="bg-gradient-to-br from-orange-500/5 to-transparent border border-orange-500/20 rounded-2xl p-6 group hover:border-orange-500/30 transition-all md:col-span-4">
                                <div className="flex items-center gap-2 mb-3">
                                    <div className="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center border border-orange-500/20">
                                        <Zap size={20} className="text-orange-400" />
                                    </div>
                                    <span className="text-xs font-bold text-orange-400/70 uppercase tracking-wider">Total Operaciones</span>
                                </div>
                            ...
                            <div className="h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                                {logs.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center p-12">
                                        <div className="w-20 h-20 rounded-2xl bg-fuchsia-500/5 flex items-center justify-center mb-4 border border-fuchsia-500/10">
                                            <Flame size={32} className="text-fuchsia-400/50" />
                                        </div>
