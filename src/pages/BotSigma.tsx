...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Shield, BarChart3, Target, Clock, CheckCircle2, XCircle, AlertCircle, ArrowRight, DollarSign } from 'lucide-react';
...
const BotSigma = () => {
    ...
    const { isRunning, stats, logs, startBot, stopBot } = useBotSigma();

    // Config States
    const [stake, setStake] = useState<string>('0.35');
    const [stopLoss, setStopLoss] = useState<string>('58.00');
    ...
    const handleToggleBot = () => {
        if (isRunning) {
            stopBot();
            toast.info('Sigma Scalper detenido');
        } else {
            if (!isConnected) {
            ...
            if (success) {
                toast.success('Sigma Scalper iniciado');
            }
        }
    };
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-purple-600 flex items-center justify-center border border-purple-400/20">
                                <Target className="text-white" size={28} />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Sigma Scalper</h1>
                            <p className="text-sm text-white/40">Estrategia de dígitos con alta precisión</p>
                        </div>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            {isRunning && (
                                <span className="absolute inset-0 rounded-full bg-purple-500 animate-ping" />
                            )}
                            <span className={cn(
                                "absolute inset-0 rounded-full",
                                isRunning ? "bg-purple-500" : "bg-gray-500"
                            )} />
                        </div>
                        <span className={cn(
                            "text-sm font-bold uppercase tracking-wider",
                            isRunning ? "text-purple-400" : "text-white/50"
                        )}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-purple-500/50 focus:ring-2 focus:ring-purple-500/20 transition-all outline-none disabled:opacity-50"
                                        step="0.01"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-purple-500/50 transition-all outline-none disabled:opacity-50"
                                    >
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isRunning
                                    ...
                                    : "bg-gradient-to-r from-purple-600 to-purple-500 hover:from-purple-500 hover:to-purple-600 text-white border-none shadow-lg shadow-purple-500/25"
                            )}
                        >
                            {isRunning ? (
                                <>
                            ...
                            <div className="h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                                {logs.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center p-12">
                                        <div className="w-20 h-20 rounded-2xl bg-purple-500/5 flex items-center justify-center mb-4 border border-purple-500/10">
