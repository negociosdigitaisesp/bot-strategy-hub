# FASE 1: GERAÇÃO - Estratégia #63

## Dados da Matriz
- **ID**: 63
- **Nome**: Detrended Price Burst
- **Categoria**: Oscillator / Turbo
...
## 📋 CRIAÇÃO DE ESTRATÉGIA #63 - Detrended Price Burst

### 1. NOME & DESCRIÇÃO
**Nome**: DPO Burst V3.0
**Descrição**: Estratégia baseada no Detrended Price Oscillator (DPO) para identificar ciclos de preço removendo a tendência. Ideal para V25 que tem ciclos curtos e repetitivos.

### 2. INDICADORES (máx 2 - PERFIL TURBO)
1. **DPO (14)**: Price - SMA(14) deslocado 7 períodos
   - Remove tendência, mostra apenas ciclos
2. **Zero Line**: Referência para cruzamentos

### 3. REGRAS DE ENTRADA CALL
**Setup**: DPO Bullish Reversal
- DPO cruza ACIMA de zero (vindo de território negativo)
- DPO estava < -0.5 no candle anterior (oversold cíclico)

### 4. REGRAS DE ENTRADA PUT
**Setup**: DPO Bearish Reversal
- DPO cruza ABAIXO de zero (vindo de território positivo)
- DPO estava > 0.5 no candle anterior (overbought cíclico)

### 5. EXPIRATION
**60 segundos (1 minuto)** - TURBO
Justificativa: V25 tem ciclos rápidos. DPO captura o início da reversão cíclica.

### 6. FREQUENCY TUNING (ATINGIR 20 SINAIS/HORA)
**Perfil TURBO → 20/h**:
- **Cooldown**: 60 segundos
- **Thresholds**: DPO zero cross ocorre frequentemente em V25
- Filtro de extremo (±0.5) garante qualidade

### 7. FILTROS POR TIER (TIER 3 - AGGRESSIVE)
- **Regime**: Universal (DPO funciona em qualquer regime)
- **Safety**: Evitar sinais se DPO estiver oscilando entre -0.2 e 0.2 (zona neutra)

### 8. CONFIDENCE SCORE
```
Base: 50
+15: DPO vindo de extremo (< -1.0 ou > 1.0)
+10: Cruzamento com momentum (diferença > 0.3)
+5: Candle de reversão (wick na direção oposta)

-10: DPO na zona neutra
```

### 9. BACKTESTING MENTAL
- **Win Rate**: 55-58% em V25
- **Risco**: V25 pode ter tendências que ignoram ciclos
- **Mitigação**: Filtro de extremo

### 10. JUSTIFICATIVA TÉCNICA
DPO foi criado por William Blau para isolar ciclos removendo a tendência. É superior ao RSI para identificar reversões cíclicas em ativos sintéticos como V25.
...
class Strategy_63_DetrendedPriceBurst_V3:
    def __init__(self):
        ...
        # DPO Parameters
        self.dpo_period = 14
        self.displacement = 7  # period / 2 + 1
    ...
    def _calc_dpo(self, data):
        ...
        # Align: DPO uses price from displacement periods ago
        # Simplified: use current price - displaced SMA
        if len(sma) < self.displacement: return np.zeros(0)
        
        # Price array aligned with SMA
        price_aligned = data[-len(sma):]
        
        # Displace SMA backward (use older SMA values)
