...
export const supabaseOracle: SupabaseClient = createClient(
  ...
  {
    db: {
      // CORRETO: tabelas estão no schema hft_lake conforme PRD_DATA_LAKE.md
      // Para o schema ser acessível via API: Supabase Dashboard → Settings → API → Extra schemas → adicionar "hft_lake"
      schema: 'hft_lake'
    },
    auth: {
...
export const testOracleConnection = async (): Promise<boolean> => {
  ...
  try {
    ...
    console.log('[ORACLE][CONNECTION TEST] OK — public schema acessível')
    return true
  } catch (err) {
