...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Shield, BarChart3, CheckCircle2, XCircle, AlertCircle, ArrowRight } from 'lucide-react';
import { cn } from '../lib/utils';
import { toast } from 'sonner';
import { useBotSX } from '../hooks/useBotSX';
import { useDeriv } from '../contexts/DerivContext';

export const BotTerminal = () => {
    const { isConnected } = useDeriv();
    const { isRunning, stats, logs, startBot, stopBot } = useBotSX();
    ...
    const [martingale, setMartingale] = useState<string>('2.0');
    ...
    const handleToggleBot = () => {
        if (isRunning) {
            ...
            if (isNaN(stopLossVal) || stopLossVal <= 0) {
                toast.error('Stop Loss inválido');
                return;
            }
            if (isNaN(takeProfitVal) || takeProfitVal <= 0) {
                toast.error('Take Profit inválido');
                return;
            }
            ...
            const success = startBot({
                ...
                martingaleFactor: martingaleVal || 1,
                symbol: 'R_10',
                growthRate: 0.02,
            });

            if (success) {
                toast.success('Bot Alpha iniciado');
            }
        }
    };
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center border border-primary/20">
                                <Zap className="text-white" size={28} />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Bot Alpha</h1>
                            <p className="text-sm text-white/40">Estrategia de acumulación con patrón Blue-Red-Red-Red</p>
                        </div>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            <span className={cn(
                                ...
                                isRunning ? "bg-primary" : "bg-gray-500"
                            )} />
                        </div>
                        <span className={cn(
                            "text-sm font-bold uppercase tracking-wider",
                            isRunning ? "text-primary" : "text-white/50"
                        )}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all outline-none disabled:opacity-50"
                                        step="0.01"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-primary/50 transition-all outline-none disabled:opacity-50"
                                        step="0.1"
                                    />
                                </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isRunning
                                    ...
                                    : "bg-gradient-to-r from-primary to-primary/80 hover:from-primary/90 hover:to-primary text-white border-none shadow-lg shadow-primary/25"
                            )}
                        >
                            {isRunning ? (
                                <>
