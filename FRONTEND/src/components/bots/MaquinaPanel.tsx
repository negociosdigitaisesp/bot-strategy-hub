...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Target, CheckCircle2, XCircle, AlertCircle, ArrowRight, DollarSign } from 'lucide-react';
import { cn } from '../../lib/utils';
import { toast } from 'sonner';
import { useBotMaquina } from '../../hooks/useBotMaquina';
import { useDeriv } from '../../contexts/DerivContext';

interface MaquinaPanelProps {
    isActive: boolean;
    onToggle: () => void;
    onBack: () => void;
}

export const MaquinaPanel: React.FC<MaquinaPanelProps> = ({ isActive, onToggle, onBack }) => {
    const { isConnected } = useDeriv();
    const { isRunning, stats, logs, startBot, stopBot } = useBotMaquina();
    ...
    const [martingale, setMartingale] = useState<string>('2.5');
    ...
    const handleToggleBot = () => {
        if (isRunning) {
            stopBot();
            toast.info('Maquina del Ganancias detenido');
        } else {
            if (!isConnected) {
            ...
            const success = startBot({
                ...
                martingaleFactor: martingaleVal || 2.5,
                symbol: 'R_75',
            });

            if (success) {
                toast.success('Maquina del Ganancias iniciado');
            }
        }
    };
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 flex items-center justify-center border border-amber-400/20">
                                <Zap className="text-white" size={28} />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Maquina del Ganancias</h1>
                            <p className="text-sm text-white/40">Estrategia Even/Odd basada en 3 dígitos</p>
                        </div>
                    </div>
                    ...
                    <div className={cn(
                        ...
                        <div className={cn(
                            ...
                            <span className={cn(
                                ...
                                isRunning ? "bg-amber-500" : "bg-gray-500"
                            )} />
                        </div>
                        <span className={cn(
                            "text-sm font-bold uppercase tracking-wider",
                            isRunning ? "text-amber-400" : "text-white/50"
                        )}>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/20 transition-all outline-none disabled:opacity-50"
                                        step="0.01"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-amber-500/50 transition-all outline-none disabled:opacity-50"
                                        step="0.1"
                                    />
                                </div>
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isRunning
                                    ...
                                    : "bg-gradient-to-r from-amber-600 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-white border-none shadow-lg shadow-amber-500/25"
                            )}
                        >
                            {isRunning ? (
                                <>
                            ...
                            <div className="h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                                {logs.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center p-12">
                                        <div className="w-20 h-20 rounded-2xl bg-amber-500/5 flex items-center justify-center mb-4 border border-amber-500/10">
