...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Target, BarChart3, CheckCircle2, XCircle, AlertCircle, ArrowRight, Rocket } from 'lucide-react';
import { cn } from '../lib/utils';
import { toast } from 'sonner';
import { useBotSpeed } from '../hooks/useBotSpeed';
import { useDeriv } from '../contexts/DerivContext';

const BotSpeed = () => {
    const { isConnected } = useDeriv();
    const { isRunning, stats, logs, startBot, stopBot } = useBotSpeed();

    // Config States
    const [stake, setStake] = useState<string>('0.35');
    const [stopLoss, setStopLoss] = useState<string>('10.00');
    const [takeProfit, setTakeProfit] = useState<string>('5.00');
    const [martingale, setMartingale] = useState<string>('2');
    const [prediction, setPrediction] = useState<string>('1');
    const [maxStake, setMaxStake] = useState<string>('10');
    ...
    const handleToggleBot = () => {
        if (isRunning) {
            ...
            const maxStakeVal = parseFloat(maxStake);
            ...
            const success = startBot({
                ...
                martingaleSize: martingaleVal || 2,
                symbol: 'R_75',
                prediction: predictionVal || 1,
                maxStake: maxStakeVal || undefined,
            });

            if (success) {
                toast.success('Speed Bot iniciado - Modo Automático');
            }
        }
    };
    ...
    const getLogIcon = (type: string) => {
        switch (type) {
            ...
            default: return <ArrowRight size={16} className="text-violet-400" />;
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#0a0a14] via-[#10081a] to-[#0a0a14] p-6">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center border border-violet-400/20">
                                <Rocket className="text-white" size={28} />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Speed Bot</h1>
                            <p className="text-sm text-white/40">Martingale Clásico R_75 (Automático)</p>
                        </div>
                    </div>
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <p className="text-sm text-yellow-200/80">
                        <strong>Modo Automático:</strong> Este bot opera continuamente sin esperar un dígito específico. Asegúrese de configurar correctamente Stop Loss y Take Profit.
                    </p>
                </div>
                ...
                <div className="grid grid-cols-12 gap-6">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        step="0.01"
                                    />
                                    <p className="text-[10px] text-violet-400/80 mt-1.5 font-medium flex items-center gap-1">
                                        <Zap size={10} />
                                        Recomendado: 0.35
                                    </p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={isRunning}
                                        className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-violet-500/50 transition-all outline-none disabled:opacity-50"
                                    >
                        ...
                        <button
                            ...
                            className={cn(
                                ...
                                isRunning
                                    ...
                                    : "bg-gradient-to-r from-violet-600 to-purple-500 hover:from-violet-500 hover:to-purple-600 text-white border-none shadow-lg shadow-violet-500/25"
                            )}
                        >
                            {isRunning ? (
                                <>
                    ...
                    <div className="col-span-12 lg:col-span-9 space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                            ...
                            {/* Win Rate */}
                            <div className="bg-gradient-to-br from-purple-500/5 to-transparent border border-purple-500/20 rounded-2xl p-6 group hover:border-purple-500/30 transition-all">
                                <div className="flex items-center gap-2 mb-3">
                                    <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center border border-purple-500/20">
                                        <Target size={20} className="text-purple-400" />
                                    </div>
                                    <span className="text-xs font-bold text-purple-400/70 uppercase tracking-wider">Win Rate</span>
                                </div>
                            ...
                            {/* Total Operations */}
                            <div className="bg-gradient-to-br from-fuchsia-500/5 to-transparent border border-fuchsia-500/20 rounded-2xl p-6 group hover:border-fuchsia-500/30 transition-all md:col-span-4">
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    <span className="text-xs font-bold text-fuchsia-400/70 uppercase tracking-wider">Total Operaciones</span>
                                </div>
                                ...
                                {logs.length === 0 ? (
                                    <div className="h-full flex flex-col items-center justify-center p-12">
                                        <div className="w-20 h-20 rounded-2xl bg-violet-500/5 flex items-center justify-center mb-4 border border-violet-500/10">
                                            <Rocket size={32} className="text-violet-400/50" />
                                        </div>
                                        ...
                                        {logs.map((log, index) => (
                                            <div
                                                ...
                                                className={cn(
                                                    ...
                                                    log.type === 'info' && "bg-violet-500/5 border border-violet-500/20 hover:border-violet-500/30"
                                                )}
