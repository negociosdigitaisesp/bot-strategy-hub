...
import { Play, Square, Settings2, Activity, TrendingUp, Zap, Target, BarChart3, CheckCircle2, XCircle, AlertCircle, ArrowRight, Lock } from 'lucide-react';
import { cn } from '../lib/utils';
import { toast } from 'sonner';
import { useBotDoubleCuentas } from '../hooks/useBotDoubleCuentas';
import { useDeriv } from '../contexts/DerivContext';

const DoubleCuentas = () => {
  const { isConnected } = useDeriv();
  const { isRunning, stats, logs, startBot, stopBot } = useBotDoubleCuentas();

  // Config States
  const [stake, setStake] = useState<string>('0.35');
  const [stopLoss, setStopLoss] = useState<string>('8.50');
  const [takeProfit, setTakeProfit] = useState<string>('4.00');
  const [martingale, setMartingale] = useState<string>('1.8');
  ...
  const handleToggleBot = () => {
    if (isRunning) {
      stopBot();
      toast.info('Bot Double Cuentas detenido');
    } else {
      if (!isConnected) {
      ...
      const success = startBot({
        ...
        martingaleFactor: martingaleVal || 1.8,
        symbol: 'R_75',
        prediction: predictionVal || 3,
      });

      if (success) {
        toast.success('Bot Double Cuentas iniciado');
      }
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a0f18] via-[#0f141f] to-[#0a0f18] p-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <div className="relative w-14 h-14 rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-600 flex items-center justify-center border border-blue-400/20">
                <Zap className="text-white" size={28} />
              </div>
            </div>
            <div>
              <h1 className="text-3xl font-black text-white mb-1 tracking-tight">Double Cuentas</h1>
              <p className="text-sm text-white/40">Estrategia Conservadora R_75 (Digit Over)</p>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            <div className={cn(
              ...
              <span className={cn(
                ...
                isRunning ? "bg-blue-500" : "bg-gray-500"
              )} />
            </div>
            <span className={cn(
              "text-sm font-bold uppercase tracking-wider",
              isRunning ? "text-blue-400" : "text-white/50"
            )}>
        ...
        <div className="grid grid-cols-12 gap-6">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    disabled={isRunning}
                    className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-blue-500/50 focus:ring-2 focus:ring-blue-500/20 transition-all outline-none disabled:opacity-50"
                    step="0.01"
                  />
                  <p className="text-[10px] text-cyan-400/80 mt-1.5 font-medium flex items-center gap-1">
                    <Zap size={10} />
                    Recomiendo empezar con 0.35
                  </p>
                </div>

                <div>
                  <label className="text-xs text-white/40 uppercase tracking-wider mb-2 block font-medium flex items-center justify-between">
                    Last Digit (Trigger)
                  ...
                  <input
                    ...
                    value="0"
                    disabled={true}
                    className="w-full px-4 py-3 bg-white/5 border border-white/5 rounded-xl text-white/50 font-mono cursor-not-allowed"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    disabled={isRunning}
                    className="w-full px-4 py-3 bg-black/40 border border-white/10 rounded-xl text-white font-mono focus:border-blue-500/50 transition-all outline-none disabled:opacity-50"
                  >
            ...
            <button
              ...
              className={cn(
                ...
                isRunning
                  ...
                  : "bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-600 text-white border-none shadow-lg shadow-blue-500/25"
              )}
            >
              {isRunning ? (
                <>
          ...
          <div className="col-span-12 lg:col-span-9 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              {/* Win Rate */}
              <div className="bg-gradient-to-br from-cyan-500/5 to-transparent border border-cyan-500/20 rounded-2xl p-6 group hover:border-cyan-500/30 transition-all">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-cyan-500/10 flex items-center justify-center border border-cyan-500/20">
                    <Target size={20} className="text-cyan-400" />
                  </div>
                  <span className="text-xs font-bold text-cyan-400/70 uppercase tracking-wider">Win Rate</span>
                </div>
              ...
              {/* Total Operations */}
              <div className="bg-gradient-to-br from-indigo-500/5 to-transparent border border-indigo-500/20 rounded-2xl p-6 group hover:border-indigo-500/30 transition-all md:col-span-4">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-10 h-10 rounded-xl bg-indigo-500/10 flex items-center justify-center border border-indigo-500/20">
                    <Zap size={20} className="text-indigo-400" />
                  </div>
                  <span className="text-xs font-bold text-indigo-400/70 uppercase tracking-wider">Total Operaciones</span>
                </div>
              ...
              <div className="h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                {logs.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center p-12">
                    <div className="w-20 h-20 rounded-2xl bg-blue-500/5 flex items-center justify-center mb-4 border border-blue-500/10">
                      <Target size={32} className="text-blue-400/50" />
                    </div>
